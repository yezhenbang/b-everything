...
void aasort_out_of_core(uint32_t an, float32_t* a, uint32_t bn, float32_t* b, float32_t* out) {
...
void aasort(uint32_t n, float32_t* in, float32_t* out) {
    /*
    ...
    for (uint32_t i = 0; i < n; i += block_elements) {
        uint32_t k = std::min(n - i, block_elements);

        /*
         * (2) Sort each block with the in-core sorting algorithm.
         */
        if (!aasort_in_core(k, in + i, out + i))
            return;
    }
    ...
    while (block_elements < n) {
        for (uint32_t i = 0; i < n; i += block_elements * 2) {
            if (n - i <= block_elements) {
                ...
                uint32_t bn = std::min(n - (i + block_elements), block_elements);
                aasort_out_of_core(an, tin + i, bn, tin + i + block_elements, tout + i);
            }
        }
...
void* aasort_naive_parallel_run(void* data) {
    ...
    if (params->n <= 1) {
        *(params->out) = *(params->in);
        ...
        aasort_out_of_core(
            nl, ( __m128i* )params->in, nu, ( __m128i* )&(params->in[nl]), ( __m128i*
)params->out);
    }
...
int main(int argc, char const* argv[]) {
    static const std::vector< int > K = {16};
    static const std::vector< uint > N = {
        65536, 1 << 10, 1 << 14, 1 << 16, 1 << 18, 1 << 20, 40000};
    ...
    for (auto cache : CACHE) {
        ...
        for (auto n : N) {
            ...
            for (const auto& k : K) {
                ...
                int time_aa = measure_us([&]() { aasort(n, input.data(), output.data()); }, 10);
                // input = random_data;
                // int time_stl = measure_us(
                // [&]() { std::sort(input.begin(), input.end(), std::greater< float32_t >());
                // });
                // std::cout << "std_sort: N-" << n << " :" << time_stl << "us" << std::endl;
                std::cout << "aa_sort: N-" << n << " :" << time_aa << "us" << std::endl;
                if (!is_sorted(output, std::greater_equal< float32_t >())) {
